...
Room.prototype.updateFrameStep = function(delta_time) {
    ...
    this.checkCollisionOfTanks(zone_tank_arr, zone_explosion_arr);
        ...
        for (var j = 0; j < tank_arr.length; j++) {
            ...
            if ((bullet.player_id !== tank.id) && bullet.checkCollisionWithTank(tank)) {
                ...
                if (this.PLAYER_LIST.hasOwnProperty(bullet.player_id)) {
                    ...
                    shooter.fireOnTarget(); //award the shooter
                }
                tank.beShooted(shooter_id); // reduce hp, set the shooter
            }
        }
    }
 ...
 */
Room.prototype.checkCollisionOfTanks = function(zone_tank_arr, zone_explosion_arr){
                ...
                if (other_tank.id !== current_tank.id && (!other_tank.is_collided_with_other_tank)) {
                    if (Utils.distace2Object(current_tank.pos, other_tank.pos) < 100) {
                        if (current_tank.checkCollisionWithOtherTank(other_tank)) {
                            ...
                            var explosion = new Explosion((current_tank.pos.x + other_tank.pos.x) / 2,
                                (current_tank.pos.y + other_tank.pos.y) / 2, (current_tank.hp + other_tank.hp) / 2,
                                this.count_explosion);

                            //list of explosion all over the map
                            this.EXPLOSION_LIST[explosion.id] = explosion;

                            //mark to not dupplicate
                            ...
                            //put the explosion into the righ zone
    ...
    for (var key in this.PLAYER_LIST) {
        if (key < 0) {
            ...
            if (boot.is_shooted && boot.shooter_id !== 0) {                
                if (this.PLAYER_LIST.hasOwnProperty(boot.shooter_id)) {
...
Room.prototype.createNewBullet = function (tank) {
    this.count_bullet++;	
    if (this.count_bullet > 1000000) {
        this.count_bullet = 1;
    }
    var shooting_angle = tank.gun_angle;
